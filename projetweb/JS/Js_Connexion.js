function validerConnexion() {
    ...
    if (XmlHttp == null) {
        ...
        XmlHttp.onreadystatechange = stateChangedConnexion;
...
function stateChangedLogin() {
    if (XmlHttp.readyState == 4) {
        ...
        if (json.find != "ok") {
            document.getElementById('messageErrorLogin').style.display = "none";
        } else {
        }
    }
}

function stateChangedConnexion()
{
    if (XmlHttp.readyState == 4) {
        json = JSON.parse(XmlHttp.responseText);
        if (json.find == "ok") {
...
function inscription() {
    ...
    var param = "action=inscri&pwd=" + md5(pass) + "&log=" + acc + "&email=" + email;
    if (acc == "" || pass == "" || email == "" || pass.length < 6 || !reg.test(email) ){ 
...
function stateChangedInscription()
{
    if (XmlHttp.readyState == 4) {
        ...
        if (json.inscri == "ok") {
            ...
            document.getElementById('messageErrorLogin').style.display = "block";
            setTimeout(function() {
                document.getElementById('messageErrorLogin').style.display = "none";
            }, 3000);
            document.getElementById('messageError').style.display = "none";
...
/*
 * Méthode équivalente à la fonction md5 en PHP
 * Récupéré sur http://phpjs.org/functions/md5/
 * 
 */
function md5(str) {
